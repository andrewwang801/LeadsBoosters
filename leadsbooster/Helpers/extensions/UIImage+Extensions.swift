...
//  Cattivo VIA
//
//  Created by Swift Coder on 9/30/16.
...
extension UIImage {
    ...
    func jpegRepresentation(compressionQuality q:CGFloat) -> Data?{
        ...
        return UIImage(cgImage: cgImage).jpegRepresentation(compressionQuality: q)
    }
    ...
    func pngRepresentation() -> Data?{
        ...
        return UIImage(cgImage: cgImage).pngRepresentation()
    }
}
...
// MARK: - Draw Image over this image
extension UIImage {
    /**
     Draw image over self at Point with Size
     - Parameter image : UIImage to be drawn
     - Parameter atPoint : Point to be drawn
     - Parameter size : Size of image to be drawn, if nil then will use image's size
     - Returns new UIImage with overlaid image
     */
    func draw(image:UIImage, atPoint:CGPoint, size:CGSize? = nil) -> UIImage{
        // Check scale
        UIGraphicsBeginImageContextWithOptions(self.size, false, self.scale)
        draw(in: CGRect(x: 0, y: 0, width: self.size.width, height: self.size.height))
        
        let overlaySize:CGSize = size ?? (image.size * (self.scale / image.scale))
        
        image.draw(in: CGRect(origin: atPoint, size: overlaySize))
        let image2Return = UIGraphicsGetImageFromCurrentImageContext()
        UIGraphicsEndImageContext()
        return image2Return!
    }
    
    /**
     Draws text over image at Point
     - Parameter text : Text to be drawn
     - Parameter atPoint : CGPoint
     - Parameter attributes : [String:AnyObject]? attribue of string
     */
    func draw(text:String, atPoint point:CGPoint, withAttributes attrs:[NSAttributedString.Key:AnyObject]? = nil) -> UIImage {
        // Check scale
        UIGraphicsBeginImageContextWithOptions(self.size, false, self.scale)
        draw(in: CGRect(x: 0, y: 0, width: self.size.width, height: self.size.height))
...
extension UIImage {
    // MARK: - Rotate Image in Radians.
    func rotate(inRadians radians:CGFloat, flipOverHorizontalAxis fh:Bool = false, verticalAxis fv:Bool = false) -> UIImage?{
        
        guard let bmCGImage = cgImage_ else {
            return nil
        }
        ...
        guard let bmContext = CGContext(data: nil, width: Int(rotatedRect.width), height: Int(rotatedRect.height), bitsPerComponent: 8, bytesPerRow: 0, space: CGColorSpaceCreateDeviceRGB(), bitmapInfo: bmInfo) else {
            return nil
        }
        ...
        // Rotation happens here (around the center)
        bmContext.translateBy(x: +(rotatedRect.size.width / 2.0), y: +(rotatedRect.size.height / 2.0))
        bmContext.rotate(by: radians)
        
        // Do flips
        bmContext.scaleBy(x: fh ? -1.0 : 1.0, y: fv ? -1.0 : 1.0)
        
        
        // Draw the image in the bitmap context
        bmContext.draw(bmCGImage, in: CGRect(x: -(width / 2.0), y: -(height / 2.0), width: width, height: height))
        
        // Create am image object from the context
...
extension UIImage {
    func save(to:URL) throws {
        guard let scaledImage = scaled(toMaxDimension: 1024),
            let data = scaledImage.jpegRepresentation(compressionQuality: 8.0) else {
                throw AppError.writeImageFailed
        }
...
extension UIImage {
    ...
    func itemImage() -> UIImage?{
        return self.scaleToSize(25.0)?.withRenderingMode(.alwaysOriginal)
    }
}
